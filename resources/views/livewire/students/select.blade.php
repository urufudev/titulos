        ...
        <div class="col-md-5 col-12">
            <div class="form-group">
                ...
                @if ($typeinsti == 'TECNOLOGICO')
                    {{ Form::select('title_level', [null => 'Selecciona'] + ['TECNICO' => 'TECNICO', 'PROFESIONAL TECNICO' => 'PROFESIONAL TECNICO', 'PROFESIONAL' => 'PROFESIONAL'], null, ['class' => 'form-control', 'autocomplete' => 'off']) }}
                @elseif($typeinsti == 'PEDAGOGICO')
               ...
               {{--  @if ($typeinsti == 'TECNOLOGICO')
                ...
                @elseif($typeinsti == 'PEDAGOGICO')
                    aeax
                @endif --}}
                {{-- @foreach ($this->level_tecnologico as $level)
                        <option value="{{ $level->id }}">{{ $carrer->name }}</option>
                    @endforeach 
                    {{-- {{dd($institutes)}}
                    {{--  --}}
                    {{-- {{Form::select($level_tecnologico,null,['class'=>'form-control','autocomplete'=>'off'])}} --}}
                @error('title_level')
                    <div class="text-danger fs-15">{{ $message }}</div>
                @enderror
            </div>
