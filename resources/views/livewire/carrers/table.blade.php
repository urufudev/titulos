          ...
          <table  class="table table-hover mb-1 ">
            <thead>
              <tr class="font-medium-2" style="cursor:pointer">
                ...
                <th wire:click.prevent="sortBy('description')" class="font-medium-1">
                    ...
                    DESCRIPCCIÓN @include('partials._sort-icon',['field'=>'description'])
            ...
            <tbody>
              ...
              <tr class="font-medium-2"> 
                <td class="text-center align-middle"><b>{{ $carrer->id }}</b> </td>
                <td class="align-middle">{{ $carrer->name }}</td>
                <td class="align-middle">{{ $carrer->description }}</td>
                ...
                <td class="text-bold-600 text-center">
                    <div class="btn-group" role="group" aria-label="Basic example">
                      @can('update', $carrer)    
                      <a href="{{route('carrers.edit',$carrer->id)}}" class="btn btn-icon btn-success" data-toggle="tooltip" data-placement="top" data-original-title="Editar">
                          <i class="bx bx-edit"></i>
                        </a>
                       @endcan 
                        @can('forceDelete', $carrer)
                        @if($laboral->status=='ACTIVO')
                        <a style="cursor:pointer" data-target="#modal-delete-{{$carrer->id}}" data-toggle="modal" class="btn btn-icon btn-danger" data-placement="top" data-toggle="tooltip" title="Eliminar" >
                          <i class="bx bx-trash-alt white"></i>
                        </a>
                        
                        @elseif($laboral->status=='INACTIVO')
                        <a style="cursor:pointer" data-target="#modal-delete-{{$carrer->id}}" data-toggle="modal" class="btn btn-icon btn-danger" data-placement="top" data-toggle="tooltip" title="Eliminar" >
                          <i class="bx bx-check-alt white"></i>
                        </a>
...
{{--                 Viewing {{ $carrers->firstItem() }} - {{ $carrers->lastItem() }} of {{ $carrers->total() }} entries
 --}}
                {{ $carrers->links('partials.custom-pagination') }}
            </div>
